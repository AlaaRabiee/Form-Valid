...
let passField=passRegex.test(passwordField.value)
...
passwordField.addEventListener("input", function() {
    let passisValid = passRegex.test(passwordField.value);
    let outputList5 = passwordField.parentElement.querySelector("ul"); // Select the corresponding <ul> for output

    outputList5.innerHTML = "";

    if (!passisValid) {
        outputList5.innerHTML = `
            <li class="invalid-text">Password must be num or letters %&* , only</li>
            <li class="invalid-text">Password must be only 8 digits</li>
        `;
    } else {
        outputList5.innerHTML = `<li class="valid-text">Valid password</li>`;
    }
});
